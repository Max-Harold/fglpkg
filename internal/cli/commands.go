...
`,
	},
	{
		...
		Usage: "fglpkg publish [--dry-run] [--ci] [--private|--public] [--changelog <text> | --changelog-file <path>]",
		Long: `FLAGS:
  ...
  --changelog-file <path>  Read this version's changelog from <path> verbatim

Builds the package zip, uploads it, and submits the version for admin review.

CHANGELOG:
  When neither --changelog nor --changelog-file is given, publish looks for a
  CHANGELOG.md in the project root and sends the section whose heading names the
  version being published (Keep a Changelog format, e.g. "## [1.2.0]"). If the
  file exists but has no entry for the version, the changelog is sent empty and
